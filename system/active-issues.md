...
- **Ghost-write cascade — 103+ commits ahead of origin/main** — Tick branches claim Tier-A writes that never merge to main. ADR-0033 confirmed on main (`decisions/0033-passive-income-portfolio-abstraction.md`). FR-0038/39/40/41 Status lines fixed on main (tick-2026-05-23T04-49Z). FR-0042 claimed in events (`supervisor/friction/FR-0042-synthesis-erofs-misdiagnosis.md`) but not written to disk — ghost-write confirmed. Requires Tier-B-auto authority decision or attended merge playbook. Branch divergence: 103 ahead, 2 behind origin/main (doctor 2026-05-23T16:49Z).
- **Synthesis dispatch obligation — 55 cycles, INBOX at 79 items (saturation active)** — INBOX holds 79 items (4 URGENTs + 75 proposals); consumption rate 0. INBOX saturation suppression active (>30 items, per CLAUDE.md). Cycle-54 dispatch unconsumed (`runtime/.handoff/general-synthesis-cycle54-dispatch-2026-05-23T16-49Z.md`); cycles 52 and 53 also unconsumed. Tier-B-auto or attended session required.
- **reflect.sh:193 argument ordering — 9th cycle, ATTENDED SESSION ACTIONABLE** — `scripts/lib/reflect.sh:193` misorders args; CURRENT_STATE.md uncommitted across 5 projects (atlas 22 cycles+). EROFS blocks tick edits but NOT attended sessions (reflection write artifacts in scripts/lib/ prove writable for non-tick processes). Fix: move `-- CURRENT_STATE.md` after `-m` flags. First action of next attended session. Cycle-53 P1.
- **Test artifact files blocking breach detector — 9th false-positive cycle** — `scripts/lib/.erofs-test-meta-reflection` and `scripts/lib/TEST_WRITE_2951547` written by a reflection write-test (proved EROFS=false) persist as untracked files. Breach detector fires URGENT every tick (FR-0043 false positive). Attended session must `git clean -f scripts/lib/.erofs-test-meta-reflection scripts/lib/TEST_WRITE_2951547` or equivalent to stop the noise. Direct action — no judgment required.

## Currently live

- **Harness `referencing` dep gap** — `ModuleNotFoundError: No module named 'referencing'` blocks harness migration dry-run for valuation-context and researcher-context. Two `migrate.failure` events in cycle-54 window (including one with `events: {ok: 4, bad: 1}` whose bad event is unidentified). Dispatched to skillfoundry-harness session (`runtime/.handoff/skillfoundry-harness-referencing-dep-fix-2026-05-23T22-50Z.md`). Fix: add `referencing` to harness requirements.
