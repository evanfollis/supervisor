...
- **Test artifact files blocking breach detector — 22nd false-positive cycle** — `scripts/lib/.erofs-test-meta-reflection` and `scripts/lib/TEST_WRITE_2951547` written by a reflection write-test (proved EROFS=false) persist as untracked files. Breach detector fires URGENT every tick (FR-0043 false positive). Attended session must `git clean -f scripts/lib/.erofs-test-meta-reflection scripts/lib/TEST_WRITE_2951547` or equivalent to stop the noise. Direct action — no judgment required. Two consecutive attended sessions skipped this (FR-0044).
