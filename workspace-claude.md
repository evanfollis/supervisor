...
- **Local workspace names resolve locally first.** If Evan names a workspace artifact, repo, decision, queue, product, or substrate (`context-repository`, canon, atlas, skillfoundry, command, synaplex, pressure queue, etc.), search local front doors and decisions before treating the phrase as a public concept. Minimum lookup path: `/opt/workspace/CLAUDE.md`, `supervisor/decisions/`, the relevant project `README.md` / `CURRENT_STATE.md`, then external sources only after the local meaning is grounded.
...
- **`eventType` must distinguish failures from designed throttling (S1-P2 addendum).** `eventType: "failure"` is reserved for genuinely unrecoverable errors. Designed rate-limiting, cap hits, and expected backpressure must emit `eventType: "throttled"` (or `"info"` for purely informational events). Meta-scan, S3-P2 monitors, and adversarial review automation all key on `eventType` — a `failure` on designed throttling is a false positive that degrades the escalation surface.
...
- **Read `/opt/workspace/runtime/.meta/LATEST_SYNTHESIS` before trusting workspace-state claims** in `active-issues.md` or `verified-state.md` that carry a STALE banner or are older than their freshness gate. The synthesis is live-verified every ~12h; during unattended stretches it is the only accurate map. (ADR-0037 — deliberately NOT in `context-always-load`: it would blow the 30KB cap, and generated files must not be fence-injected.)
...
- **Dispatch obligation.** When a synthesis proposal lands in `runtime/.meta/cross-cutting-*.md`, the executive must dispatch a delegated project session within 7d (or before any external deadline named in the proposal, if sooner) — via `runtime/.handoff/<project>-*.md` handoff — or record an explicit deferral reason in `supervisor/decisions/` or `runtime/.meta/`. Synthesis proposals sitting past that window without dispatched action or recorded deferral escalate as FR-class structural issues. (24h→7d per ADR-0037: 28 consecutive false breaches at observed contact cadence.) The reflection/synthesis loop is a work queue, not a diagnostic archive; treating proposals as read-and-file produces the "80h perfect-diagnosis-zero-execution" failure class the loop was designed to prevent.
