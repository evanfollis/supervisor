...
skip_with_reason() {
  ...
  if [[ -f "$EVENT_FILE" ]]; then
    ...
    if (( consecutive >= 3 )); then
      ...
      # S3-P1: write INBOX file so the executive session sees the escalation.
      # FR-0043: dedupe by reason-hash so a multi-hour outage produces one
      # file with an updated counter, not N near-identical URGENTs. The Apr
      # 21–23 outage produced 29 self-similar escalations before this patch.
      mkdir -p "$SUP/handoffs/INBOX"
      reason_hash=$(printf '%s' "$reason" | sha256sum | awk '{print substr($1,1,10)}')
      URGENT_FILE="$SUP/handoffs/INBOX/URGENT-tick-escalation-${reason_hash}.md"
...
The tick has skipped ${consecutive} consecutive times with the same blocking reason.
...
- Is a manual hold file present? (${HOLD_FILE})
...
# Exclude untracked files (`??`) and the generated verified-state snapshot.
# Append-only events now live under runtime, so operational telemetry no
# longer dirties the governance repo. The remaining modified/staged files can
# represent attended-session work and still block the tick.
