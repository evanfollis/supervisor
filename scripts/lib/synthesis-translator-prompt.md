...
2. **For each proposal, decide:**
   - **Run this read-only preflight before deciding to emit:**
     ...
     3. Search `/opt/workspace/supervisor/decisions/` for that identifier and read every matching decision passage. Also follow and read every ADR explicitly referenced by the target file or its path/configuration helpers; a governing decision need not name the leaf script. For workspace-charter, context-loading, governance, or architecture proposals, this decision search is mandatory even if the target file appears to permit the change. For any proposal involving logs, telemetry, event ordering, or repo-versus-runtime state, reading ADR-0012 and ADR-0043 is mandatory before deciding.
     4. Resolve every relevant indirect path or configuration variable through its sourced helpers to the canonical location. Verify causal premises against that resolved location: a write under `/opt/workspace/runtime/` cannot dirty or advance the Git HEAD of `/opt/workspace/supervisor/`, and moving such a write before or after a Git commit does not change that fact. Treat a proposal based on the opposite premise as stale.
     5. Search the target repo for the proposed end state or an explicit rejection of it.
     6. Apply ADR-0047 requirement provenance: distinguish the capability being proposed from any vendor, product, paper, or architecture used only as an example. If a proposal promotes an external example into an evaluation subject, runtime dependency, credential, or spend target, require an accepted Decision or explicit principal instruction authorizing that exact promotion. Otherwise skip it as an unauthorized external objective; do not convert its missing credential into a blocker.
...
5. **Report what you did.** At the end, list each proposal by number, whether you wrote a handoff or skipped, and the reason if skipped. For each proposal, name the target state inspected and the governing ADRs read (or `none found`); never summarize an incomplete preflight as merely `verified live`. Include total count of handoffs emitted.
